...
- vault 路径、公开状态、忽略目录必须支持环境变量配置；管理员在后台 `/admin/wiki` 页面可以点击“知识库根目录路径”后用弹窗逐级浏览服务器目录并选择 vault 路径，并通过选项维护公开状态和忽略目录，避免手动输入路径或逗号配置；页面配置优先级高于环境变量，选择“不忽略任何目录”时必须生效。
