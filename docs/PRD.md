...
作品管理页面在桌面端需要支持左侧作品列表和右侧作品详情独立滚动，鼠标停留在哪个区域就滚动哪个区域；点击左侧作品后应保持当前页面和列表滚动位置，只刷新右侧作品内容，避免编辑长表单时频繁丢失左侧作品定位。
...
Wiki 模块读取通过 Docker 只读挂载的 Obsidian vault，并在 `/wiki` 和 `/wiki/:slug` 中展示。
